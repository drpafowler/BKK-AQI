'''
producer.py
fetch real time AQI data from the WAQI API and save it to an SQLite database.
...
# get the data from the WAQI API
def get_aqi_data(): 
...
def sql_connect(): 
 ...
 # consume the data from the API and store it in the SQLite database if it is new data
def consume_and_store():
    ...
    while True:
        ...
        if last_row:
            ...
            if data == last_data:
                print("Data is the same as the last entry, skipping insert and waiting 20 minutes.")
                time.sleep(1200)
                continue
        c.execute('''
